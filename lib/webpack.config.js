...
function getConfiguration(settings) {
    const globalCssRegex = /Style\/Global\/\S+\.s?css$/i;
    const globalCss = new ExtractTextPlugin({
        filename: 'css/zed-app.global.css'
    });

    const utilCssRegex = /Style\/Util\/\S+\.s?css$/i;
    ...
    let config = {
        ...
        resolve: {
            ...
            alias: {
                'ZedApp': 'src/Spryker/Zed/GuiComponentCollection/Presentation/App'
            }
        },
        ...
        module: {
            rules: [{
                ...
                test: globalCssRegex,
                loader: globalCss.extract(cssExtractSettings)
            }, {
                test: utilCssRegex,
